...
describe("buildBillingHeader", () => {
	...
	it("should build header with computed suffix", () => {
		...
		assert.strictEqual(
			header,
			"x-anthropic-billing-header: cc_version=2.1.200.616; cc_entrypoint=cli; cch=00000;"
		);
	});
	...
	it("should use fallback suffix when no user message exists", () => {
		...
		assert.match(header, /cc_version=2\.1\.200\.000/);
	});
	...
	it("should include all required header components", () => {
		...
		assert.match(header, /cc_version=2\.1\.200\.[0-9a-f]{3}/);
...
describe("integration: verified against Claude Code", () => {
	...
	it("should match Claude Code billing header for 'What day is it?'", () => {
		// Verified from Claude Code CLI 2.1.200 audit:
		// attribution header x-anthropic-billing-header: cc_version=2.1.200.616; cc_entrypoint=cli; cch=00000;
...
describe("provider payload patching", () => {
	...
	it("patches string system prompts for Anthropic requests", async () => {
		...
		assert.match(payload.system[0].text, /^x-anthropic-billing-header: cc_version=2\.1\.200\.616/);
...
describe("extension registration", () => {
	...
	it("resets cache and notifies on session start", async () => {
		...
		assert.match(header, new RegExp(`cc_version=2\\.1\\.200\\.${computeVersionSuffix("Second message")}`));
	});
